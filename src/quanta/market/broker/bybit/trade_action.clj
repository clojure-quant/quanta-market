(ns quanta.market.broker.bybit.trade-action
  (:require
   ...
   [quanta.market.util :refer [mix current-v first-match]]
...
(defrecord bybit-trade-action [opts websocket-order]
  p/trade-action
  ...
  (order-create! [this order]
    (let [order-msg (order-create-msg order)
  ...
  (order-cancel! [this order]
    (let [order-msg (order-cancel-msg order)]
...
(defn create-trade-action
  ...
  (let [websocket-order (create-websocket2 (merge opts {:segment :trade}))]
    (bybit-trade-action. opts websocket-order)))
